...
# Title: Python3 quantitative trading algorithm server v20161211
...
webPath = str(os.path.abspath("."))  # "C:/QuantitativeTrading/QuantitativeTradingPython/src/" 服務器運行的本地硬盤根目錄，可以使用函數當前目錄：os.path.abspath(".")，函數 os.path.abspath("..") 表示目錄的上一層目錄，函數 os.path.join(os.path.abspath(".."), "/temp/") 表示拼接路徑字符串，函數 pathlib.Path(os.path.abspath("..") + "/temp/") 表示拼接路徑字符串;
            ...
            try:
                ...
                for line in lines:
                    ...
                    if isinstance(line, str) and line != "":
                        ...
                        # http 服務器運行的根目錄 webPath = "C:/QuantitativeTrading/QuantitativeTradingPython/src/";
                        if line_Key == "webPath":
                            webPath = str(line_Value)  # http 服務器運行的根目錄 webPath = "C:/QuantitativeTrading/QuantitativeTradingPython/src/";
        ...
        if i > 0:
            ...
            if isinstance(sys.argv[i], str) and sys.argv[i] != "" and sys.argv[i].find("=", 0, int(len(sys.argv[i])-1)) != -1:
                ...
                if sys.argv[i].split("=", -1)[0] == "webPath":
                    webPath = str(sys.argv[i].split("=", -1)[1])  # http 服務器運行的根目錄 webPath = "C:/QuantitativeTrading/QuantitativeTradingPython/src/";
                    # print("webPath:", webPath)
                    continue
                # http 服務器監聽的IP地址 host = "0.0.0.0";
