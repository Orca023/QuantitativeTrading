                                ...
                                if investment_method === "Long_Position_and_Short_Selling" || investment_method === "Long_Position"
                                    ...
                                    if (x_P1_previous <= P2) && (x_P1 > P2)
                                        ...
                                        if Core.Int64(i + 1) <= Core.Int64(Base.length(P1_Array))
                                            ...
                                            # y_F_Long_Position_I[13] = x2[i + 1];  # 當日總成交金額（turnover amount）;
                                            # y_F_Long_Position_I[14] = x20[i + 1];  # 當日成交量（turnover volume）換手率（turnover rate）;
                                            # y_F_Long_Position_I[15] = x21[i + 1];  # 當日每股收益（price earnings）;
                                            # y_F_Long_Position_I[16] = x22[i + 1];  # 當日每股净值（book value per share）;
                                            ...
                                            if Core.Int64(i + 1) <= Core.Int64(Base.length(P1_Array))
                                                for j = Core.Int64(i):Core.Int64(Core.Int64(Base.length(P1_Array)) - Core.Int64(1))
                                                    ...
                                                    if Core.Float64(risk_drawdown_loss_Long_Position) <= Core.Float64(P4)
                                                        ...
                                                        # y_F_Long_Position_I[13] = x2[j + 1];  # 當日總成交金額（turnover amount）;
                                                        # y_F_Long_Position_I[14] = x20[j + 1];  # 當日成交量（turnover volume）換手率（turnover rate）;
                                                        # y_F_Long_Position_I[15] = x21[j + 1];  # 當日每股收益（price earnings）;
                                                        # y_F_Long_Position_I[16] = x22[j + 1];  # 當日每股净值（book value per share）;
                                                        Base.push!(y_F_Long_Position, y_F_Long_Position_I);  # 使用 push! 函數在數組末尾追加推入新元;
                                ...
                                if investment_method === "Long_Position_and_Short_Selling" || investment_method === "Short_Selling"
                                    ...
                                    if (x_P1_previous >= P3) && (x_P1 < P3)
                                        ...
                                        if Core.Int64(i + 1) <= Core.Int64(Base.length(P1_Array))
                                            ...
                                            # y_F_Short_Selling_I[13] = x2[i + 1];  # 當日總成交金額（turnover amount）;
                                            # y_F_Short_Selling_I[14] = x20[i + 1];  # 當日成交量（turnover volume）換手率（turnover rate）;
                                            # y_F_Short_Selling_I[15] = x21[i + 1];  # 當日每股收益（price earnings）;
                                            # y_F_Short_Selling_I[16] = x22[i + 1];  # 當日每股净值（book value per share）;
                                            ...
                                            if Core.Int64(i + 1) <= Core.Int64(Base.length(P1_Array))
                                                for j = Core.Int64(i):Core.Int64(Core.Int64(Base.length(P1_Array)) - Core.Int64(1))
                                                    ...
                                                    if Core.Float64(risk_drawdown_loss_Short_Selling) <= Core.Float64(P4)
                                                        ...
                                                        # y_F_Short_Selling_I[13] = x2[j + 1];  # 當日總成交金額（turnover amount）;
                                                        # y_F_Short_Selling_I[14] = x20[j + 1];  # 當日成交量（turnover volume）換手率（turnover rate）;
                                                        # y_F_Short_Selling_I[15] = x21[j + 1];  # 當日每股收益（price earnings）;
                                                        # y_F_Short_Selling_I[16] = x22[j + 1];  # 當日每股净值（book value per share）;
                                                        Base.push!(y_F_Short_Selling, y_F_Short_Selling_I);  # 使用 push! 函數在數組末尾追加推入新元;
