...
// 使用説明：
// 控制臺命令列運行指令：
// C:\> C:\QuantitativeTrading\Nodejs\Nodejs-22.20.0\node.exe C:/QuantitativeTrading/QuantitativeTradingJavaScript/src/QuantitativeTradingServer.js configFile=C:/QuantitativeTrading/QuantitativeTradingJavaScript/config.txt webPath=C:/QuantitativeTrading/html/ host=::0 port=10001 Key=username:password number_cluster_Workers=0
// root@localhost:~# /usr/bin/node /home/QuantitativeTrading/QuantitativeTradingJavaScript/src/QuantitativeTradingServer.js configFile=/home/QuantitativeTrading/QuantitativeTradingJavaScript/config.txt webPath=/home/QuantitativeTrading/html/ host=::0 port=10001 Key=username:password number_cluster_Workers=0
...
const Router = require('./Router.js');  // 引用自定義的路由（Router）模組;  // require(require('path').join(require('path').resolve("."), "Router.js")); require('path').resolve("..").toString().concat("/temp/"); 當加載自定義的模塊時，引用模塊需要包括路徑和模塊名的完整引用，只寫模塊名會報錯;
const do_data = Router.do_data;  // 使用「Router.js」模組中的成員「do_data(data_Str)」函數, 用於處理從硬盤文檔讀取到的JSON對象數據，然後返回處理之後的結果JSON對象，即路由（Router）功能;
const do_Request_Router = Router.do_Request_Router;  // 使用「Router.js」模組中的成員「do_Request_Router(request_url, request_POST_String, request_headers, callback)」函數, 用於處理用戶端（Client）發送請求的路由（Router）功能;
const do_Response_Router = Router.do_Response_Router;  // 使用「Router.js」模組中的成員「do_Response_Router(response_status, response_headers, response_POST_String, callback)」函數, 用於處理從服務端（Server）返回的響應值數據（Response）的執行函數，即路由（Router）功能;

// // // 使用自定義類（Class）方法模型擬合運算;
// // const Model_Fitting = require('./Curve_Fitting.js').ModelFitting;
// // // console.log(ModelFitting.help);
...
// // const Roots_Equation = Curve_Fitting.RootsEquation;  // 使用「Curve_Fitting.js」模塊中的成員「RootsEquation([Y_target], f, df, X_0, tolerance = (1e-6), maxIter = 300, method = "Bisection")」函數, 用於一元方程單變量求根近似值運算（4, 5 parameter logistic model）;
// // 數值分析方法求解方程根近似值;
// const Roots_Equation = require('./Curve_Fitting.js').RootsEquation;  // 使用「Curve_Fitting.js」模塊中的成員「RootsEquation([Y_target], f, df, X_0, tolerance = (1e-6), maxIter = 300, method = "Bisection")」函數, 用於一元方程單變量求根近似值運算（4, 5 parameter logistic model）;
// // https://github.com/mljs/levenberg-marquardt
// // root@localhost:~# npm install ml-levenberg-marquardt
// // import { levenbergMarquardt } from 'ml-levenberg-marquardt';  // 導入第三方擴展包「ml-levenberg-marquardt」，用於任意形式曲缐方程擬合（Curve Fitting），需要在控制臺先安裝第三方擴展包「ml-levenberg-marquardt」：root@localhost:~# npm install ml-levenberg-marquardt 成功之後才能使用;
// // 一元三階多項式（Polynomial(Cubic)）曲缐擬合 f(x, P) = P1·x^0 + P2·x^1 + P3·x^2 + P4·x^3 ;
// const Polynomial3_fit = require('./Curve_Fitting.js').Polynomial3Fit;  // 使用「Curve_Fitting.js」模塊中的成員「Polynomial3Fit(training_data, testing_data, Pdata_0, weight, Plower Pupper)」函數, 用於一元三階多項式（Polynomial(Cubic)）曲缐擬合運算;
...
// const LC5P_fit = require('./Curve_Fitting.js').LC5PFit;  // 使用「Curve_Fitting.js」模塊中的成員「LC5PFit(training_data, testing_data, Pdata_0, weight, Plower Pupper)」函數, 用於邏輯 5 參數模型（5-parameter logistic curve）曲缐擬合運算;
// // 邏輯四參數模型（4-parameter logistic model）曲缐擬合 f(x, P) = P[4] - (P[4] - P[1])/(1.0 + (x/P[2])^P[3]) ;
// const LC4P_fit = require('./Curve_Fitting.js').LC4PFit;  // 使用「Curve_Fitting.js」模塊中的成員「LC4PFit(training_data, testing_data, Pdata_0, weight, Plower Pupper)」函數, 用於邏輯四參數模型（4-parameter logistic model）曲缐擬合運算;
...
//     };
//     if (url_search_JSON.has("algorithmUser")) {
//         algorithmUser = String(url_search_JSON.get("algorithmUser"));  // "username";  // 用戶端請求（Request）算法（Algorithm）的驗證賬號密碼;
//     };
//     if (url_search_JSON.has("algorithmPass")) {
//         algorithmPass = String(url_search_JSON.get("algorithmPass"));  // "password";  // 用戶端請求（Request）算法（Algorithm）的驗證賬號密碼;
//     };
//     if (url_search_JSON.has("algorithmName")) {
//         algorithmName = String(url_search_JSON.get("algorithmName"));  // "Polynomial3Fit";  // 用戶端請求（Request）算法（Algorithm）的名稱;
//     };
...
//             // 自定義函數判用戶端（Client）請求體 request_POST_String 是否為一個 JSON 格式的字符串;
//             // if (request_POST_String !== "" && isStringJSON(request_POST_String)) {
//                 try {
//                     if (request_POST_String !== "") {
//                         request_POST_JSON = JSON.parse(request_POST_String, true);
...
//             // 讀取並清洗模型訓練集數據;
//             let training_data = {};
//             // training_data = {"Xdata": request_POST_JSON["trainXdata"], "Ydata": request_POST_JSON["trainYdata"]};
//             if (typeof (request_POST_JSON) !== undefined && request_POST_JSON !== undefined && request_POST_JSON !== null && request_POST_JSON !== NaN) {
//                 if ((typeof (request_POST_JSON) === 'object' && Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object object]' && !(request_POST_JSON.length)) && (Object.keys(request_POST_JSON).length > 0)) {
//                     if (request_POST_JSON.hasOwnProperty("trainXdata")) {
//                         if ((Object.prototype.toString.call(request_POST_JSON["trainXdata"]).toLowerCase() === '[object array]') && (request_POST_JSON["trainXdata"].length > 0)) {
//                             for (let i = 0; i < request_POST_JSON["trainXdata"].length; i++) {
//                                 if ((Object.prototype.toString.call(request_POST_JSON["trainXdata"][i]).toLowerCase() === '[object array]') && (request_POST_JSON["trainXdata"][i].length > 0)) {
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (!(training_data.hasOwnProperty("Xdata")))) {
//                                         training_data["Xdata"] = new Array;
//                                         // training_data["Xdata"] = new Array(parseInt(request_POST_JSON["trainXdata"].length)).fill(null);
//                                     };
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (training_data.hasOwnProperty("Xdata"))) {
//                                         let trainingData_Xdata_i = new Array;
//                                         // training_data["Xdata"][i] = new Array(parseInt(request_POST_JSON["trainXdata"][i].length)).fill(NaN);
//                                         for (let j = 0; j < request_POST_JSON["trainXdata"][i].length; j++) {
//                                             if (Object.prototype.toString.call(request_POST_JSON["trainXdata"][i][j]).toLowerCase() === '[object number]') {
//                                                 trainingData_Xdata_i.push(parseFloat(request_POST_JSON["trainXdata"][i][j]));
//                                                 // training_data["Xdata"][i][j] = parseFloat(request_POST_JSON["trainXdata"][i][j]);
//                                             } else if (Object.prototype.toString.call(request_POST_JSON["trainXdata"][i][j]).toLowerCase() === '[object string]') {
//                                                 if ((request_POST_JSON["trainXdata"][i][j] === "+Base.Inf") || (request_POST_JSON["trainXdata"][i][j] === "+Inf") || (request_POST_JSON["trainXdata"][i][j] === "+inf") || (request_POST_JSON["trainXdata"][i][j] === "+Infinity") || (request_POST_JSON["trainXdata"][i][j] === "+infinity") || (request_POST_JSON["trainXdata"][i][j] === "Base.Inf") || (request_POST_JSON["trainXdata"][i][j] === "Inf") || (request_POST_JSON["trainXdata"][i][j] === "inf") || (request_POST_JSON["trainXdata"][i][j] === "Infinity") || (request_POST_JSON["trainXdata"][i][j] === "infinity")) {
//                                                     trainingData_Xdata_i.push((+Infinity));
//                                                     // training_data["Xdata"][i][j] = (+Infinity);
//                                                 } else if ((request_POST_JSON["trainXdata"][i][j] === "-Base.Inf") || (request_POST_JSON["trainXdata"][i][j] === "-Inf") || (request_POST_JSON["trainXdata"][i][j] === "-inf") || (request_POST_JSON["trainXdata"][i][j] === "-Infinity") || (request_POST_JSON["trainXdata"][i][j] === "-infinity")) {
//                                                     trainingData_Xdata_i.push((-Infinity));
//                                                     // training_data["Xdata"][i][j] = (-Infinity);
//                                                 } else {
//                                                     trainingData_Xdata_i.push(parseFloat(request_POST_JSON["trainXdata"][i][j]));
//                                                     // trainingData_Xdata_i.push(String(request_POST_JSON["trainXdata"][i][j]));
//                                                     // training_data["Xdata"][i][j] = String(request_POST_JSON["trainXdata"][i][j]);
//                                                 };
//                                             } else {
//                                                 trainingData_Xdata_i.push(request_POST_JSON["trainXdata"][i][j]);
//                                                 // training_data["Xdata"][i][j] = request_POST_JSON["trainXdata"][i][j];
//                                             };
//                                         };
//                                         training_data["Xdata"].push(trainingData_Xdata_i);
//                                         trainingData_Xdata_i = null;
//                                     };
//                                 } else if (Object.prototype.toString.call(request_POST_JSON["trainXdata"][i]).toLowerCase() === '[object number]') {
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (!(training_data.hasOwnProperty("Xdata")))) {
//                                         training_data["Xdata"] = new Array;
//                                         // training_data["Xdata"] = new Array(parseInt(request_POST_JSON["trainXdata"].length)).fill(NaN);
//                                     };
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (training_data.hasOwnProperty("Xdata"))) {
//                                         training_data["Xdata"].push(parseFloat(request_POST_JSON["trainXdata"][i]));
//                                         // training_data["Xdata"][i] = parseFloat(request_POST_JSON["trainXdata"][i]);
//                                     };
//                                 } else if (Object.prototype.toString.call(request_POST_JSON["trainXdata"][i]).toLowerCase() === '[object string]') {
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (!(training_data.hasOwnProperty("Xdata")))) {
//                                         training_data["Xdata"] = new Array;
//                                         // training_data["Xdata"] = new Array(parseInt(request_POST_JSON["trainXdata"].length)).fill(NaN);
//                                     };
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (training_data.hasOwnProperty("Xdata"))) {
//                                         if ((request_POST_JSON["trainXdata"][i] === "+Base.Inf") || (request_POST_JSON["trainXdata"][i] === "+Inf") || (request_POST_JSON["trainXdata"][i] === "+inf") || (request_POST_JSON["trainXdata"][i] === "+Infinity") || (request_POST_JSON["trainXdata"][i] === "+infinity") || (request_POST_JSON["trainXdata"][i] === "Base.Inf") || (request_POST_JSON["trainXdata"][i] === "Inf") || (request_POST_JSON["trainXdata"][i] === "inf") || (request_POST_JSON["trainXdata"][i] === "Infinity") || (request_POST_JSON["trainXdata"][i] === "infinity")) {
//                                             training_data["Xdata"].push((+Infinity));
//                                             // training_data["Xdata"][i] = (+Infinity);
//                                         } else if ((request_POST_JSON["trainXdata"][i] === "-Base.Inf") || (request_POST_JSON["trainXdata"][i] === "-Inf") || (request_POST_JSON["trainXdata"][i] === "-inf") || (request_POST_JSON["trainXdata"][i] === "-Infinity") || (request_POST_JSON["trainXdata"][i] === "-infinity")) {
//                                             training_data["Xdata"].push((-Infinity));
//                                             // training_data["Xdata"][i] = (-Infinity);
//                                         } else {
//                                             training_data["Xdata"].push(parseFloat(request_POST_JSON["trainXdata"][i]));
//                                             // training_data["Xdata"].push(String(request_POST_JSON["trainXdata"][i]));
//                                             // training_data["Xdata"][i] = String(request_POST_JSON["trainXdata"][i]);
//                                         };
//                                     };
//                                 } else {
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (!(training_data.hasOwnProperty("Xdata")))) {
...
//                                     };
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (training_data.hasOwnProperty("Xdata"))) {
//                                         training_data["Xdata"].push(request_POST_JSON["trainXdata"][i]);
//                                         // training_data["Xdata"][i] = request_POST_JSON["trainXdata"][i];
//                                     };
//                                 };
//                             };
//                         };
//                     };
//                     if (request_POST_JSON.hasOwnProperty("trainYdata")) {
//                         if ((Object.prototype.toString.call(request_POST_JSON["trainYdata"]).toLowerCase() === '[object array]') && (request_POST_JSON["trainYdata"].length > 0)) {
//                             for (let i = 0; i < request_POST_JSON["trainYdata"].length; i++) {
//                                 if ((Object.prototype.toString.call(request_POST_JSON["trainYdata"][i]).toLowerCase() === '[object array]') && (request_POST_JSON["trainYdata"][i].length > 0)) {
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (!(training_data.hasOwnProperty("Ydata")))) {
//                                         training_data["Ydata"] = new Array;
//                                         // training_data["Ydata"] = new Array(parseInt(request_POST_JSON["trainYdata"].length)).fill(null);
//                                     };
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (training_data.hasOwnProperty("Ydata"))) {
//                                         let trainingData_Ydata_i = new Array;
//                                         // training_data["Ydata"][i] = new Array(parseInt(request_POST_JSON["trainYdata"][i].length)).fill(NaN);
//                                         for (let j = 0; j < request_POST_JSON["trainYdata"][i].length; j++) {
//                                             if (Object.prototype.toString.call(request_POST_JSON["trainYdata"][i][j]).toLowerCase() === '[object number]') {
//                                                 trainingData_Ydata_i.push(parseFloat(request_POST_JSON["trainYdata"][i][j]));
//                                                 // training_data["Ydata"][i][j] = parseFloat(request_POST_JSON["trainYdata"][i][j]);
//                                             } else if (Object.prototype.toString.call(request_POST_JSON["trainYdata"][i][j]).toLowerCase() === '[object string]') {
//                                                 if ((request_POST_JSON["trainYdata"][i][j] === "+Base.Inf") || (request_POST_JSON["trainYdata"][i][j] === "+Inf") || (request_POST_JSON["trainYdata"][i][j] === "+inf") || (request_POST_JSON["trainYdata"][i][j] === "+Infinity") || (request_POST_JSON["trainYdata"][i][j] === "+infinity") || (request_POST_JSON["trainYdata"][i][j] === "Base.Inf") || (request_POST_JSON["trainYdata"][i][j] === "Inf") || (request_POST_JSON["trainYdata"][i][j] === "inf") || (request_POST_JSON["trainYdata"][i][j] === "Infinity") || (request_POST_JSON["trainYdata"][i][j] === "infinity")) {
//                                                     trainingData_Ydata_i.push((+Infinity));
//                                                     // training_data["Ydata"][i][j] = (+Infinity);
//                                                 } else if ((request_POST_JSON["trainYdata"][i][j] === "-Base.Inf") || (request_POST_JSON["trainYdata"][i][j] === "-Inf") || (request_POST_JSON["trainYdata"][i][j] === "-inf") || (request_POST_JSON["trainYdata"][i][j] === "-Infinity") || (request_POST_JSON["trainYdata"][i][j] === "-infinity")) {
//                                                     trainingData_Ydata_i.push((-Infinity));
//                                                     // training_data["Ydata"][i][j] = (-Infinity);
//                                                 } else {
//                                                     trainingData_Ydata_i.push(parseFloat(request_POST_JSON["trainYdata"][i][j]));
//                                                     // trainingData_Ydata_i.push(String(request_POST_JSON["trainYdata"][i][j]));
//                                                     // training_data["Ydata"][i][j] = String(request_POST_JSON["trainYdata"][i][j]);
//                                                 };
//                                             } else {
//                                                 trainingData_Ydata_i.push(request_POST_JSON["trainYdata"][i][j]);
//                                                 // training_data["Ydata"][i][j] = request_POST_JSON["trainYdata"][i][j];
//                                             };
//                                         };
//                                         training_data["Ydata"].push(trainingData_Ydata_i);
//                                         trainingData_Ydata_i = null;
//                                     };
//                                 } else if (Object.prototype.toString.call(request_POST_JSON["trainYdata"][i]).toLowerCase() === '[object number]') {
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (!(training_data.hasOwnProperty("Ydata")))) {
//                                         training_data["Ydata"] = new Array;
//                                         // training_data["Ydata"] = new Array(parseInt(request_POST_JSON["trainYdata"].length)).fill(NaN);
//                                     };
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (training_data.hasOwnProperty("Ydata"))) {
//                                         training_data["Ydata"].push(parseFloat(request_POST_JSON["trainYdata"][i]));
//                                         // training_data["Ydata"][i] = parseFloat(request_POST_JSON["trainYdata"][i]);
//                                     };
//                                 } else if (Object.prototype.toString.call(request_POST_JSON["trainYdata"][i]).toLowerCase() === '[object string]') {
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (!(training_data.hasOwnProperty("Ydata")))) {
//                                         training_data["Ydata"] = new Array;
//                                         // training_data["Ydata"] = new Array(parseInt(request_POST_JSON["trainYdata"].length)).fill(NaN);
//                                     };
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (training_data.hasOwnProperty("Ydata"))) {
//                                         if ((request_POST_JSON["trainYdata"][i] === "+Base.Inf") || (request_POST_JSON["trainYdata"][i] === "+Inf") || (request_POST_JSON["trainYdata"][i] === "+inf") || (request_POST_JSON["trainYdata"][i] === "+Infinity") || (request_POST_JSON["trainYdata"][i] === "+infinity") || (request_POST_JSON["trainYdata"][i] === "Base.Inf") || (request_POST_JSON["trainYdata"][i] === "Inf") || (request_POST_JSON["trainYdata"][i] === "inf") || (request_POST_JSON["trainYdata"][i] === "Infinity") || (request_POST_JSON["trainYdata"][i] === "infinity")) {
//                                             training_data["Ydata"].push((+Infinity));
//                                             // training_data["Ydata"][i] = (+Infinity);
//                                         } else if ((request_POST_JSON["trainYdata"][i] === "-Base.Inf") || (request_POST_JSON["trainYdata"][i] === "-Inf") || (request_POST_JSON["trainYdata"][i] === "-inf") || (request_POST_JSON["trainYdata"][i] === "-Infinity") || (request_POST_JSON["trainYdata"][i] === "-infinity")) {
//                                             training_data["Ydata"].push((-Infinity));
//                                             // training_data["Ydata"][i] = (-Infinity);
//                                         } else {
//                                             training_data["Ydata"].push(parseFloat(request_POST_JSON["trainYdata"][i]));
//                                             // training_data["Ydata"].push(String(request_POST_JSON["trainYdata"][i]));
//                                             // training_data["Ydata"][i] = String(request_POST_JSON["trainYdata"][i]);
//                                         };
//                                     };
//                                 } else {
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (!(training_data.hasOwnProperty("Ydata")))) {
...
//                                     };
//                                     if ((typeof (training_data) === 'object' && Object.prototype.toString.call(training_data).toLowerCase() === '[object object]' && !(training_data.length)) && (training_data.hasOwnProperty("Ydata"))) {
//                                         training_data["Ydata"].push(request_POST_JSON["trainYdata"][i]);
//                                         // training_data["Ydata"][i] = request_POST_JSON["trainYdata"][i];
//                                     };
//                                 };
//                             };
//                         };
...
//             let trainXdata = new Array;
//             // trainXdata = new Array(parseInt(training_data["Xdata"].length)).fill(NaN);
//             if (training_data.hasOwnProperty("Xdata")) {
//                 trainXdata = training_data["Xdata"];
//             };
//             // console.log(trainXdata);
//             let trainYdata = new Array;
//             // trainYdata = new Array(parseInt(training_data["Ydata"].length)).fill(NaN);
//             if (training_data.hasOwnProperty("Ydata")) {
//                 trainYdata = training_data["Ydata"];
//             };
//             // console.log(trainYdata);

//             // 計算訓練集數據的 Ydata 均值向量;
//             let trainYdataMean = new Array;
//             // trainYdataMean = new Array(parseInt(trainYdata.length)).fill(NaN);
//             for (let i = 0; i < trainYdata.length; i++) {
//                 if (Object.prototype.toString.call(trainYdata[i]).toLowerCase() === '[object array]') {
//                     let yMean = NaN;
//                     if (trainYdata[i].length === 0) {
//                         yMean = NaN;
//                     };
//                     if (trainYdata[i].length === 1) {
//                         yMean = parseFloat(trainYdata[i][0]);
//                     };
//                     if (trainYdata[i].length >= 2) {
//                         yMean = parseFloat(math.mean(trainYdata[i]));
//                     };
//                     trainYdataMean.push(yMean);
//                     // trainYdataMean[i] = yMean;
//                     yMean = NaN;
//                 } else if (Object.prototype.toString.call(trainYdata[i]).toLowerCase() === '[object number]') {
//                     trainYdataMean.push(parseFloat(trainYdata[i]));
//                     // trainYdataMean[i] = parseFloat(trainYdata[i]);
//                 } else if (Object.prototype.toString.call(trainYdata[i]).toLowerCase() === '[object string]') {
//                     if ((trainYdata[i] === "+Base.Inf") || (trainYdata[i] === "+Inf") || (trainYdata[i] === "+inf") || (trainYdata[i] === "+Infinity") || (trainYdata[i] === "+infinity") || (trainYdata[i] === "Base.Inf") || (trainYdata[i] === "Inf") || (trainYdata[i] === "inf") || (trainYdata[i] === "Infinity") || (trainYdata[i] === "infinity")) {
//                         trainYdataMean.push((+Infinity));
//                         // trainYdataMean[i] = (+Infinity);
//                     } else if ((trainYdata[i] === "-Base.Inf") || (trainYdata[i] === "-Inf") || (trainYdata[i] === "-inf") || (trainYdata[i] === "-Infinity") || (trainYdata[i] === "-infinity")) {
//                         trainYdataMean.push((-Infinity));
//                         // trainYdataMean[i] = (-Infinity);
//                     } else {
//                         trainYdataMean.push(parseFloat(trainYdata[i]));
//                         // trainYdataMean.push(String(trainYdata[i]));
//                         // trainYdataMean[i] = String(trainYdata[i]);
//                     };
//                 } else {
//                     // trainYdataMean.push(NaN);
//                     // // trainYdataMean[i] = NaN;
//                     trainYdataMean.push(trainYdata[i]);
//                     // trainYdataMean[i] = trainYdata[i];
//                 };
//             };
//             // console.log(trainYdataMean);

//             // 計算訓練集數據的 Ydata 標準差向量;
//             let trainYdataSTD = new Array;
//             // trainYdataSTD = new Array(parseInt(trainYdata.length)).fill(NaN);
//             for (let i = 0; i < trainYdata.length; i++) {
//                 if (Object.prototype.toString.call(trainYdata[i]).toLowerCase() === '[object array]') {
//                     let ySTD = NaN;
//                     if (trainYdata[i].length === 0) {
...
//                     };
//                     if (trainYdata[i].length === 2) {
//                         // if (parseFloat(math.mean(trainYdata[i])) === parseFloat(0.0)) {
//                             ySTD = parseFloat(parseFloat(math.abs(parseFloat(trainYdata[i][0]) - parseFloat(trainYdata[i][1]))) / parseInt(2));
//                         // } else {
//                         //     ySTD = parseFloat(math.abs(parseFloat(trainYdata[i][0]) - parseFloat(trainYdata[i][1]))/parseFloat(math.mean(trainYdata[i])));
//                         // };
//                     };
//                     if (trainYdata[i].length >= 3) {
//                         ySTD = parseFloat(math.std(trainYdata[i]));
//                         // ySTD = parseFloat(math.std(trainYdata[i], 'uncorrected'));
//                     };
//                     trainYdataSTD.push(ySTD);
//                     // trainYdataSTD[i] = ySTD;
//                     ySTD = null;
//                 } else if (Object.prototype.toString.call(trainYdata[i]).toLowerCase() === '[object number]') {
//                     trainYdataSTD.push(parseFloat(0.0));
//                     // trainYdataSTD[i] = parseFloat(0.0);
//                 } else if (Object.prototype.toString.call(trainYdata[i]).toLowerCase() === '[object string]') {
//                     if ((trainYdata[i] === "+Base.Inf") || (trainYdata[i] === "+Inf") || (trainYdata[i] === "+inf") || (trainYdata[i] === "+Infinity") || (trainYdata[i] === "+infinity") || (trainYdata[i] === "Base.Inf") || (trainYdata[i] === "Inf") || (trainYdata[i] === "inf") || (trainYdata[i] === "Infinity") || (trainYdata[i] === "infinity")) {
//                         trainYdataSTD.push((+Infinity));
//                         // trainYdataSTD[i] = (+Infinity);
//                     } else if ((trainYdata[i] === "-Base.Inf") || (trainYdata[i] === "-Inf") || (trainYdata[i] === "-inf") || (trainYdata[i] === "-Infinity") || (trainYdata[i] === "-infinity")) {
//                         trainYdataSTD.push((-Infinity));
//                         // trainYdataSTD[i] = (-Infinity);
//                     } else {
//                         // trainYdataSTD.push(NaN);
//                         // // trainYdataSTD[i] = NaN;
//                         trainYdataSTD.push(parseFloat(0.0));
//                         // trainYdataSTD[i] = parseFloat(0.0);
//                         // trainYdataSTD.push(String(trainYdata[i]));
//                         // // trainYdataSTD[i] = String(trainYdata[i]);
//                     };
//                 } else {
//                     // trainYdataSTD.push(NaN);
//                     // // trainYdataSTD[i] = NaN;
...
//                 };
//             };
//             // console.log(trainYdataSTD);
    
//             // 計算訓練集數據的 testXdata 均值向量;
//             let trainXdataMean = new Array;
//             // trainXdataMean = new Array(parseInt(trainXdata.length)).fill(NaN);
//             for (let i = 0; i < trainXdata.length; i++) {
//                 if (Object.prototype.toString.call(trainXdata[i]).toLowerCase() === '[object array]') {
//                     let xMean = NaN;
//                     if (trainXdata[i].length === 0) {
//                         xMean = NaN;
//                     };
//                     if (trainXdata[i].length === 1) {
//                         xMean = parseFloat(trainXdata[i][0]);
//                     };
//                     if (trainXdata[i].length >= 2) {
//                         xMean = parseFloat(math.mean(trainXdata[i]));
//                     };
//                     trainXdataMean.push(xMean);
//                     // trainXdataMean[i] = xMean;
//                     xMean = null;
//                 } else if (Object.prototype.toString.call(trainXdata[i]).toLowerCase() === '[object number]') {
//                     trainXdataMean.push(parseFloat(trainXdata[i]));
//                     // trainXdataMean[i] = parseFloat(trainXdata[i]);
//                 } else if (Object.prototype.toString.call(trainXdata[i]).toLowerCase() === '[object string]') {
//                     if ((trainXdata[i] === "+Base.Inf") || (trainXdata[i] === "+Inf") || (trainXdata[i] === "+inf") || (trainXdata[i] === "+Infinity") || (trainXdata[i] === "+infinity") || (trainXdata[i] === "Base.Inf") || (trainXdata[i] === "Inf") || (trainXdata[i] === "inf") || (trainXdata[i] === "Infinity") || (trainXdata[i] === "infinity")) {
//                         trainXdataMean.push((+Infinity));
//                         // trainXdataMean[i] = (+Infinity);
//                     } else if ((trainXdata[i] === "-Base.Inf") || (trainXdata[i] === "-Inf") || (trainXdata[i] === "-inf") || (trainXdata[i] === "-Infinity") || (trainXdata[i] === "-infinity")) {
//                         trainXdataMean.push((-Infinity));
//                         // trainXdataMean[i] = (-Infinity);
//                     } else {
//                         trainXdataMean.push(parseFloat(trainXdata[i]));
//                         // trainXdataMean.push(String(trainXdata[i]));
//                         // trainXdataMean[i] = String(trainXdata[i]);
//                     };
//                 } else {
//                     // trainXdataMean.push(NaN);
//                     // // trainXdataMean[i] = NaN;
//                     trainXdataMean.push(trainXdata[i]);
//                     // trainXdataMean[i] = trainXdata[i];
//                 };
//             };
//             // console.log(trainXdataMean);

//             // 計算訓練集數據的 testXdata 標準差向量;
//             let trainXdataSTD = new Array;  // 求解 y 的 1·SD 值的根;
//             // trainXdataSTD = new Array(parseInt(trainXdata.length)).fill(NaN);
//             for (let i = 0; i < trainXdata.length; i++) {
//                 if (Object.prototype.toString.call(trainXdata[i]).toLowerCase() === '[object array]') {
//                     let xSTD = NaN;
//                     if (trainXdata[i].length === 0) {
...
//                     };
//                     if (trainXdata[i].length === 2) {
//                         // if (parseFloat(math.mean(trainXdata[i])) === parseFloat(0.0)) {
//                             xSTD = parseFloat(parseFloat(math.abs(parseFloat(trainXdata[i][0]) - parseFloat(trainXdata[i][1]))) / parseInt(2));
//                         // } else {
//                         //     xSTD = parseFloat(math.abs(parseFloat(trainXdata[i][0]) - parseFloat(trainXdata[i][1]))/parseFloat(math.mean(trainXdata[i])));
//                         // };
//                     };
//                     if (trainXdata[i].length >= 3) {
//                         xSTD = parseFloat(math.std(trainXdata[i]));
//                         // xSTD = parseFloat(math.std(trainXdata[i], 'uncorrected'));
//                     };
//                     trainXdataSTD.push(xSTD);
//                     // trainXdataSTD[i] = xSTD;
//                     xSTD = null;
//                 } else if (Object.prototype.toString.call(trainXdata[i]).toLowerCase() === '[object number]') {
//                     trainXdataSTD.push(parseFloat(0.0));
//                     // trainXdataSTD[i] = parseFloat(0.0);
//                 } else if (Object.prototype.toString.call(trainXdata[i]).toLowerCase() === '[object string]') {
//                     if ((trainXdata[i] === "+Base.Inf") || (trainXdata[i] === "+Inf") || (trainXdata[i] === "+inf") || (trainXdata[i] === "+Infinity") || (trainXdata[i] === "+infinity") || (trainXdata[i] === "Base.Inf") || (trainXdata[i] === "Inf") || (trainXdata[i] === "inf") || (trainXdata[i] === "Infinity") || (trainXdata[i] === "infinity")) {
//                         trainXdataSTD.push((+Infinity));
//                         // trainXdataSTD[i] = (+Infinity);
//                     } else if ((trainXdata[i] === "-Base.Inf") || (trainXdata[i] === "-Inf") || (trainXdata[i] === "-inf") || (trainXdata[i] === "-Infinity") || (trainXdata[i] === "-infinity")) {
//                         trainXdataSTD.push((-Infinity));
//                         // trainXdataSTD[i] = (-Infinity);
//                     } else {
//                         // trainXdataSTD.push(NaN);
//                         // // trainXdataSTD[i] = NaN;
//                         trainXdataSTD.push(parseFloat(0.0));
//                         // trainXdataSTD[i] = parseFloat(0.0);
//                         // trainXdataSTD.push(String(trainXdata[i]));
//                         // // trainXdataSTD[i] = String(trainXdata[i]);
//                     };
//                 } else {
//                     // trainXdataSTD.push(NaN);
//                     // // trainXdataSTD[i] = NaN;
...
//                 };
//             };
//             // console.log(trainXdataSTD);

//             // 讀取並清洗模型測試集數據;
//             let testing_data = {};
//             // testing_data = {"Xdata": request_POST_JSON["testXdata"], "Ydata": request_POST_JSON["testYdata"]};
//             if (typeof (request_POST_JSON) !== undefined && request_POST_JSON !== undefined && request_POST_JSON !== null && request_POST_JSON !== NaN) {
//                 if ((typeof (request_POST_JSON) === 'object' && Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object object]' && !(request_POST_JSON.length)) && (Object.keys(request_POST_JSON).length > 0)) {
//                     if (request_POST_JSON.hasOwnProperty("testXdata")) {
//                         if ((Object.prototype.toString.call(request_POST_JSON["testXdata"]).toLowerCase() === '[object array]') && (request_POST_JSON["testXdata"].length > 0)) {
//                             for (let i = 0; i < request_POST_JSON["testXdata"].length; i++) {
//                                 if ((Object.prototype.toString.call(request_POST_JSON["testXdata"][i]).toLowerCase() === '[object array]') && (request_POST_JSON["testXdata"][i].length > 0)) {
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (!(testing_data.hasOwnProperty("Xdata")))) {
//                                         testing_data["Xdata"] = new Array;
//                                         // testing_data["Xdata"] = new Array(parseInt(request_POST_JSON["testXdata"].length)).fill(null);
//                                     };
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (testing_data.hasOwnProperty("Xdata"))) {
//                                         let testData_Xdata_i = new Array;
//                                         // testing_data["Xdata"][i] = new Array(parseInt(request_POST_JSON["testXdata"][i].length)).fill(NaN);
//                                         for (let j = 0; j < request_POST_JSON["testXdata"][i].length; j++) {
//                                             if (Object.prototype.toString.call(request_POST_JSON["testXdata"][i][j]).toLowerCase() === '[object number]') {
//                                                 testData_Xdata_i.push(parseFloat(request_POST_JSON["testXdata"][i][j]));
//                                                 // testing_data["Xdata"][i][j] = parseFloat(request_POST_JSON["testXdata"][i][j]);
//                                             } else if (Object.prototype.toString.call(request_POST_JSON["testXdata"][i][j]).toLowerCase() === '[object string]') {
//                                                 if ((request_POST_JSON["testXdata"][i][j] === "+Base.Inf") || (request_POST_JSON["testXdata"][i][j] === "+Inf") || (request_POST_JSON["testXdata"][i][j] === "+inf") || (request_POST_JSON["testXdata"][i][j] === "+Infinity") || (request_POST_JSON["testXdata"][i][j] === "+infinity") || (request_POST_JSON["testXdata"][i][j] === "Base.Inf") || (request_POST_JSON["testXdata"][i][j] === "Inf") || (request_POST_JSON["testXdata"][i][j] === "inf") || (request_POST_JSON["testXdata"][i][j] === "Infinity") || (request_POST_JSON["testXdata"][i][j] === "infinity")) {
//                                                     testData_Xdata_i.push((+Infinity));
//                                                     // testing_data["Xdata"][i][j] = (+Infinity);
//                                                 } else if ((request_POST_JSON["testXdata"][i][j] === "-Base.Inf") || (request_POST_JSON["testXdata"][i][j] === "-Inf") || (request_POST_JSON["testXdata"][i][j] === "-inf") || (request_POST_JSON["testXdata"][i][j] === "-Infinity") || (request_POST_JSON["testXdata"][i][j] === "-infinity")) {
//                                                     testData_Xdata_i.push((-Infinity));
//                                                     // testing_data["Xdata"][i][j] = (-Infinity);
//                                                 } else {
//                                                     testData_Xdata_i.push(parseFloat(request_POST_JSON["testXdata"][i][j]));
//                                                     // testData_Xdata_i.push(String(request_POST_JSON["testXdata"][i][j]));
//                                                     // testing_data["Xdata"][i][j] = String(request_POST_JSON["testXdata"][i][j]);
//                                                 };
//                                             } else {
//                                                 testData_Xdata_i.push(request_POST_JSON["testXdata"][i][j]);
//                                                 // testing_data["Xdata"][i][j] = request_POST_JSON["testXdata"][i][j];
//                                             };
//                                         };
//                                         testing_data["Xdata"].push(testData_Xdata_i);
//                                         testData_Xdata_i = null;
//                                     };
//                                 } else if (Object.prototype.toString.call(request_POST_JSON["testXdata"][i]).toLowerCase() === '[object number]') {
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (!(testing_data.hasOwnProperty("Xdata")))) {
//                                         testing_data["Xdata"] = new Array;
//                                         // testing_data["Xdata"] = new Array(parseInt(request_POST_JSON["testXdata"].length)).fill(NaN);
//                                     };
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (testing_data.hasOwnProperty("Xdata"))) {
//                                         testing_data["Xdata"].push(parseFloat(request_POST_JSON["testXdata"][i]));
//                                         // testing_data["Xdata"][i] = parseFloat(request_POST_JSON["testXdata"][i]);
//                                     };
//                                 } else if (Object.prototype.toString.call(request_POST_JSON["testXdata"][i]).toLowerCase() === '[object string]') {
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (!(testing_data.hasOwnProperty("Xdata")))) {
//                                         testing_data["Xdata"] = new Array;
//                                         // testing_data["Xdata"] = new Array(parseInt(request_POST_JSON["testXdata"].length)).fill(NaN);
//                                     };
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (testing_data.hasOwnProperty("Xdata"))) {
//                                         if ((request_POST_JSON["testXdata"][i] === "+Base.Inf") || (request_POST_JSON["testXdata"][i] === "+Inf") || (request_POST_JSON["testXdata"][i] === "+inf") || (request_POST_JSON["testXdata"][i] === "+Infinity") || (request_POST_JSON["testXdata"][i] === "+infinity") || (request_POST_JSON["testXdata"][i] === "Base.Inf") || (request_POST_JSON["testXdata"][i] === "Inf") || (request_POST_JSON["testXdata"][i] === "inf") || (request_POST_JSON["testXdata"][i] === "Infinity") || (request_POST_JSON["testXdata"][i] === "infinity")) {
//                                             testing_data["Xdata"].push((+Infinity));
//                                             // testing_data["Xdata"][i] = (+Infinity);
//                                         } else if ((request_POST_JSON["testXdata"][i] === "-Base.Inf") || (request_POST_JSON["testXdata"][i] === "-Inf") || (request_POST_JSON["testXdata"][i] === "-inf") || (request_POST_JSON["testXdata"][i] === "-Infinity") || (request_POST_JSON["testXdata"][i] === "-infinity")) {
//                                             testing_data["Xdata"].push((-Infinity));
//                                             // testing_data["Xdata"][i] = (-Infinity);
//                                         } else {
//                                             testing_data["Xdata"].push(parseFloat(request_POST_JSON["testXdata"][i]));
//                                             // testing_data["Xdata"].push(String(request_POST_JSON["testXdata"][i]));
//                                             // testing_data["Xdata"][i] = String(request_POST_JSON["testXdata"][i]);
//                                         };
//                                     };
//                                 } else {
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (!(testing_data.hasOwnProperty("Xdata")))) {
...
//                                     };
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (testing_data.hasOwnProperty("Xdata"))) {
//                                         testing_data["Xdata"].push(request_POST_JSON["testXdata"][i]);
//                                         // testing_data["Xdata"][i] = request_POST_JSON["testXdata"][i];
//                                     };
//                                 };
//                             };
//                         };
//                     };
//                     if (request_POST_JSON.hasOwnProperty("testYdata")) {
//                         if ((Object.prototype.toString.call(request_POST_JSON["testYdata"]).toLowerCase() === '[object array]') && (request_POST_JSON["testYdata"].length > 0)) {
//                             for (let i = 0; i < request_POST_JSON["testYdata"].length; i++) {
//                                 if ((Object.prototype.toString.call(request_POST_JSON["testYdata"][i]).toLowerCase() === '[object array]') && (request_POST_JSON["testYdata"][i].length > 0)) {
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (!(testing_data.hasOwnProperty("Ydata")))) {
//                                         testing_data["Ydata"] = new Array;
//                                         // testing_data["Ydata"] = new Array(parseInt(request_POST_JSON["testYdata"].length)).fill(null);
//                                     };
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (testing_data.hasOwnProperty("Ydata"))) {
//                                         let testData_Ydata_i = new Array;
//                                         // testing_data["Ydata"][i] = new Array(parseInt(request_POST_JSON["testYdata"][i].length)).fill(NaN);
//                                         for (let j = 0; j < request_POST_JSON["testYdata"][i].length; j++) {
//                                             if (Object.prototype.toString.call(request_POST_JSON["testYdata"][i][j]).toLowerCase() === '[object number]') {
//                                                 testData_Ydata_i.push(parseFloat(request_POST_JSON["testYdata"][i][j]));
//                                                 // testing_data["Ydata"][i][j] = parseFloat(request_POST_JSON["testYdata"][i][j]);
//                                             } else if (Object.prototype.toString.call(request_POST_JSON["testYdata"][i][j]).toLowerCase() === '[object string]') {
//                                                 if ((request_POST_JSON["testYdata"][i][j] === "+Base.Inf") || (request_POST_JSON["testYdata"][i][j] === "+Inf") || (request_POST_JSON["testYdata"][i][j] === "+inf") || (request_POST_JSON["testYdata"][i][j] === "+Infinity") || (request_POST_JSON["testYdata"][i][j] === "+infinity") || (request_POST_JSON["testYdata"][i][j] === "Base.Inf") || (request_POST_JSON["testYdata"][i][j] === "Inf") || (request_POST_JSON["testYdata"][i][j] === "inf") || (request_POST_JSON["testYdata"][i][j] === "Infinity") || (request_POST_JSON["testYdata"][i][j] === "infinity")) {
//                                                     testData_Ydata_i.push((+Infinity));
//                                                     // testing_data["Ydata"][i][j] = (+Infinity);
//                                                 } else if ((request_POST_JSON["testYdata"][i][j] === "-Base.Inf") || (request_POST_JSON["testYdata"][i][j] === "-Inf") || (request_POST_JSON["testYdata"][i][j] === "-inf") || (request_POST_JSON["testYdata"][i][j] === "-Infinity") || (request_POST_JSON["testYdata"][i][j] === "-infinity")) {
//                                                     testData_Ydata_i.push((-Infinity));
//                                                     // testing_data["Ydata"][i][j] = (-Infinity);
//                                                 } else {
//                                                     testData_Ydata_i.push(parseFloat(request_POST_JSON["testYdata"][i][j]));
//                                                     // testData_Ydata_i.push(String(request_POST_JSON["testYdata"][i][j]));
//                                                     // testing_data["Ydata"][i][j] = String(request_POST_JSON["testYdata"][i][j]);
//                                                 };
//                                             } else {
//                                                 testData_Ydata_i.push(request_POST_JSON["testYdata"][i][j]);
//                                                 // testing_data["Ydata"][i][j] = request_POST_JSON["testYdata"][i][j];
//                                             };
//                                         };
//                                         testing_data["Ydata"].push(testData_Ydata_i);
//                                         testData_Ydata_i = null;
//                                     };
//                                 } else if (Object.prototype.toString.call(request_POST_JSON["testYdata"][i]).toLowerCase() === '[object number]') {
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (!(testing_data.hasOwnProperty("Ydata")))) {
//                                         testing_data["Ydata"] = new Array;
//                                         // testing_data["Ydata"] = new Array(parseInt(request_POST_JSON["testYdata"].length)).fill(NaN);
//                                     };
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (testing_data.hasOwnProperty("Ydata"))) {
//                                         testing_data["Ydata"].push(parseFloat(request_POST_JSON["testYdata"][i]));
//                                         // testing_data["Ydata"][i] = parseFloat(request_POST_JSON["testYdata"][i]);
//                                     };
//                                 } else if (Object.prototype.toString.call(request_POST_JSON["testYdata"][i]).toLowerCase() === '[object string]') {
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (!(testing_data.hasOwnProperty("Ydata")))) {
//                                         testing_data["Ydata"] = new Array;
//                                         // testing_data["Ydata"] = new Array(parseInt(request_POST_JSON["testYdata"].length)).fill(NaN);
//                                     };
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (testing_data.hasOwnProperty("Ydata"))) {
//                                         if ((request_POST_JSON["testYdata"][i] === "+Base.Inf") || (request_POST_JSON["testYdata"][i] === "+Inf") || (request_POST_JSON["testYdata"][i] === "+inf") || (request_POST_JSON["testYdata"][i] === "+Infinity") || (request_POST_JSON["testYdata"][i] === "+infinity") || (request_POST_JSON["testYdata"][i] === "Base.Inf") || (request_POST_JSON["testYdata"][i] === "Inf") || (request_POST_JSON["testYdata"][i] === "inf") || (request_POST_JSON["testYdata"][i] === "Infinity") || (request_POST_JSON["testYdata"][i] === "infinity")) {
//                                             testing_data["Ydata"].push((+Infinity));
//                                             // testing_data["Ydata"][i] = (+Infinity);
//                                         } else if ((request_POST_JSON["testYdata"][i] === "-Base.Inf") || (request_POST_JSON["testYdata"][i] === "-Inf") || (request_POST_JSON["testYdata"][i] === "-inf") || (request_POST_JSON["testYdata"][i] === "-Infinity") || (request_POST_JSON["testYdata"][i] === "-infinity")) {
//                                             testing_data["Ydata"].push((-Infinity));
//                                             // testing_data["Ydata"][i] = (-Infinity);
//                                         } else {
//                                             testing_data["Ydata"].push(parseFloat(request_POST_JSON["testYdata"][i]));
//                                             // testing_data["Ydata"].push(String(request_POST_JSON["testYdata"][i]));
//                                             // testing_data["Ydata"][i] = String(request_POST_JSON["testYdata"][i]);
//                                         };
//                                     };
//                                 } else {
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (!(testing_data.hasOwnProperty("Ydata")))) {
...
//                                     };
//                                     if ((typeof (testing_data) === 'object' && Object.prototype.toString.call(testing_data).toLowerCase() === '[object object]' && !(testing_data.length)) && (testing_data.hasOwnProperty("Xdata"))) {
//                                         testing_data["Ydata"].push(request_POST_JSON["testYdata"][i]);
//                                         // testing_data["Ydata"][i] = request_POST_JSON["testYdata"][i];
//                                     };
//                                 };
//                             };
//                         };
...
//             let testXdata = new Array;
//             // testXdata = new Array(parseInt(testing_data["Xdata"].length)).fill(NaN);
//             if (testing_data.hasOwnProperty("Xdata")) {
//                 testXdata = testing_data["Xdata"];
//             };
//             // console.log(testXdata);
//             let testYdata = new Array;
//             // testYdata = new Array(parseInt(testing_data["Ydata"].length)).fill(NaN);
//             if (testing_data.hasOwnProperty("Ydata")) {
//                 testYdata = testing_data["Ydata"];
//             };
//             // console.log(testYdata);

//             // 計算測試集數據的 testYdata 均值向量;
//             let testYdataMean = new Array;
//             // testYdataMean = new Array(parseInt(testYdata.length)).fill(NaN);
//             for (let i = 0; i < testYdata.length; i++) {
//                 if (Object.prototype.toString.call(testYdata[i]).toLowerCase() === '[object array]') {
//                     let testYMean = NaN;
//                     if (testYdata[i].length === 0) {
//                         testYMean = NaN;
//                     };
//                     if (testYdata[i].length === 1) {
//                         testYMean = parseFloat(testYdata[i][0]);
//                     };
//                     if (testYdata[i].length >= 2) {
//                         testYMean = parseFloat(math.mean(testYdata[i]));
//                     };
//                     testYdataMean.push(testYMean);
//                     // testYdataMean[i] = testYMean;
//                     testYMean = NaN;
//                 } else if (Object.prototype.toString.call(testYdata[i]).toLowerCase() === '[object number]') {
//                     testYdataMean.push(parseFloat(testYdata[i]));
//                     // testYdataMean[i] = parseFloat(testYdata[i]);
//                 } else if (Object.prototype.toString.call(testYdata[i]).toLowerCase() === '[object string]') {
//                     if ((testYdata[i] === "+Base.Inf") || (testYdata[i] === "+Inf") || (testYdata[i] === "+inf") || (testYdata[i] === "+Infinity") || (testYdata[i] === "+infinity") || (testYdata[i] === "Base.Inf") || (testYdata[i] === "Inf") || (testYdata[i] === "inf") || (testYdata[i] === "Infinity") || (testYdata[i] === "infinity")) {
//                         testYdataMean.push((+Infinity));
//                         // testYdataMean[i] = (+Infinity);
//                     } else if ((testYdata[i] === "-Base.Inf") || (testYdata[i] === "-Inf") || (testYdata[i] === "-inf") || (testYdata[i] === "-Infinity") || (testYdata[i] === "-infinity")) {
//                         testYdataMean.push((-Infinity));
//                         // testYdataMean[i] = (-Infinity);
//                     } else {
//                         testYdataMean.push(parseFloat(testYdata[i]));
//                         // testYdataMean.push(String(testYdata[i]));
//                         // testYdataMean[i] = String(testYdata[i]);
//                     };
//                 } else {
//                     // testYdataMean.push(NaN);
//                     // // testYdataMean[i] = NaN;
//                     testYdataMean.push(testYdata[i]);
//                     // testYdataMean[i] = testYdata[i];
//                 };
//             };
//             // console.log(testYdataMean);

//             // 計算測試集數據的 testYdata 標準差向量;
//             let testYdataSTD = new Array;
//             // testYdataSTD = new Array(parseInt(testYdata.length)).fill(NaN);
//             for (let i = 0; i < testYdata.length; i++) {
//                 if (Object.prototype.toString.call(testYdata[i]).toLowerCase() === '[object array]') {
//                     let testYSTD = NaN;
//                     if (testYdata[i].length === 0) {
...
//                     };
//                     if (testYdata[i].length === 2) {
//                         // if (parseFloat(math.mean(testYdata[i])) === parseFloat(0.0)) {
//                             testYSTD = parseFloat(parseFloat(math.abs(parseFloat(testYdata[i][0]) - parseFloat(testYdata[i][1]))) / parseInt(2));
//                         // } else {
//                         //     testYSTD = parseFloat(math.abs(parseFloat(testYdata[i][0]) - parseFloat(testYdata[i][1]))/parseFloat(math.mean(testYdata[i])));
//                         // };
//                     };
//                     if (testYdata[i].length >= 3) {
//                         testYSTD = parseFloat(math.std(testYdata[i]));
//                         // testYSTD = parseFloat(math.std(testYdata[i], 'uncorrected'));
//                     };
//                     testYdataSTD.push(testYSTD);
//                     // testYdataSTD[i] = testYSTD;
//                     testYSTD = null;
//                 } else if (Object.prototype.toString.call(testYdata[i]).toLowerCase() === '[object number]') {
//                     testYdataSTD.push(parseFloat(0.0));
//                     // testYdataSTD[i] = parseFloat(0.0);
//                 } else if (Object.prototype.toString.call(testYdata[i]).toLowerCase() === '[object string]') {
//                     if ((testYdata[i] === "+Base.Inf") || (testYdata[i] === "+Inf") || (testYdata[i] === "+inf") || (testYdata[i] === "+Infinity") || (testYdata[i] === "+infinity") || (testYdata[i] === "Base.Inf") || (testYdata[i] === "Inf") || (testYdata[i] === "inf") || (testYdata[i] === "Infinity") || (testYdata[i] === "infinity")) {
//                         testYdataSTD.push((+Infinity));
//                         // testYdataSTD[i] = (+Infinity);
//                     } else if ((testYdata[i] === "-Base.Inf") || (testYdata[i] === "-Inf") || (testYdata[i] === "-inf") || (testYdata[i] === "-Infinity") || (testYdata[i] === "-infinity")) {
//                         testYdataSTD.push((-Infinity));
//                         // testYdataSTD[i] = (-Infinity);
//                     } else {
//                         // testYdataSTD.push(NaN);
//                         // // testYdataSTD[i] = NaN;
//                         testYdataSTD.push(parseFloat(0.0));
//                         // testYdataSTD[i] = parseFloat(0.0);
//                         // testYdataSTD.push(String(testYdata[i]));
//                         // // testYdataSTD[i] = String(testYdata[i]);
//                     };
//                 } else {
//                     // testYdataSTD.push(NaN);
//                     // // testYdataSTD[i] = NaN;
...
//                 };
//             };
//             // console.log(testYdataSTD);

//             // 計算測試集數據的 testXdata 均值向量;
//             let testXdataMean = new Array;
//             // testXdataMean = new Array(parseInt(testXdata.length)).fill(NaN);
//             for (let i = 0; i < testXdata.length; i++) {
//                 if (Object.prototype.toString.call(testXdata[i]).toLowerCase() === '[object array]') {
//                     let testXMean = NaN;
//                     if (testXdata[i].length === 0) {
//                         testXMean = NaN;
//                     };
//                     if (testXdata[i].length === 1) {
//                         testXMean = parseFloat(testXdata[i][0]);
//                     };
//                     if (testXdata[i].length >= 2) {
//                         testXMean = parseFloat(math.mean(testXdata[i]));
//                     };
//                     testXdataMean.push(testXMean);
//                     // testXdataMean[i] = testXMean;
//                     testXMean = null;
//                 } else if (Object.prototype.toString.call(testXdata[i]).toLowerCase() === '[object number]') {
//                     testXdataMean.push(parseFloat(testXdata[i]));
//                     // testXdataMean[i] = parseFloat(testXdata[i]);
//                 } else if (Object.prototype.toString.call(testXdata[i]).toLowerCase() === '[object string]') {
//                     if ((testXdata[i] === "+Base.Inf") || (testXdata[i] === "+Inf") || (testXdata[i] === "+inf") || (testXdata[i] === "+Infinity") || (testXdata[i] === "+infinity") || (testXdata[i] === "Base.Inf") || (testXdata[i] === "Inf") || (testXdata[i] === "inf") || (testXdata[i] === "Infinity") || (testXdata[i] === "infinity")) {
//                         testXdataMean.push((+Infinity));
//                         // testXdataMean[i] = (+Infinity);
//                     } else if ((testXdata[i] === "-Base.Inf") || (testXdata[i] === "-Inf") || (testXdata[i] === "-inf") || (testXdata[i] === "-Infinity") || (testXdata[i] === "-infinity")) {
//                         testXdataMean.push((-Infinity));
//                         // testXdataMean[i] = (-Infinity);
//                     } else {
//                         testXdataMean.push(parseFloat(testXdata[i]));
//                         // testXdataMean.push(String(testXdata[i]));
//                         // testXdataMean[i] = String(testXdata[i]);
//                     };
//                 } else {
//                     // testXdataMean.push(NaN);
//                     // // testXdataMean[i] = NaN;
//                     testXdataMean.push(testXdata[i]);
//                     // testXdataMean[i] = testXdata[i];
//                 };
//             };
...
//             // testXdataSTD = new Array(parseInt(testXdata.length)).fill(NaN);
//             for (let i = 0; i < testXdata.length; i++) {
//                 if (Object.prototype.toString.call(testXdata[i]).toLowerCase() === '[object array]') {
//                     let testXSTD = NaN;
//                     if (testXdata[i].length === 0) {
...
//                     };
//                     if (testXdata[i].length === 2) {
//                         // if (parseFloat(math.mean(testXdata[i])) === parseFloat(0.0)) {
//                             testXSTD = parseFloat(parseFloat(math.abs(parseFloat(testXdata[i][0]) - parseFloat(testXdata[i][1]))) / parseInt(2));
//                         // } else {
//                         //     testXSTD = parseFloat(math.abs(parseFloat(testXdata[i][0]) - parseFloat(testXdata[i][1]))/parseFloat(math.mean(testXdata[i])));
//                         // };
//                     };
//                     if (testXdata[i].length >= 3) {
//                         testXSTD = parseFloat(math.std(testXdata[i]));
//                         // testXSTD = parseFloat(math.std(testXdata[i], 'uncorrected'));
//                     };
...
//                         testXdataSTD.push(parseFloat(0.0));
//                         // testXdataSTD[i] = parseFloat(0.0);
//                         // testXdataSTD.push(String(testXdata[i]));
//                         // // testXdataSTD[i] = String(testXdata[i]);
//                     };
//                 } else {
//                     // testXdataSTD.push(NaN);
//                     // // testXdataSTD[i] = NaN;
...
//             // // 參數擬合迭代起始值;
//             // let Pdata_0 = new Array;
//             // // Pdata_0 = [
//             // //     math.mean(math.mean(trainYdataMean.map(function(value, index, self) { let newItem = ((trainYdataMean[index] % (math.mean(trainYdataMean.map(function(value, index, self) { let newItem = parseFloat(trainYdataMean[index] / trainXdataMean[index] ** 1); return newItem; })) * trainXdataMean[index] ** 1)) * (math.mean(trainYdataMean.map(function(value, index, self) { let newItem = parseFloat(trainYdataMean[index] / trainXdataMean[index] ** 1); return newItem; })) * trainXdataMean[index] ** 1)); return newItem; }).filter(function(value, index, self) { if ((index === self.findIndex((item, index, self) => { if (item === math.min(self)) { return true; } else { return false; }; })) || (index === self.findIndex((item, index, self) => { if (item === math.max(self)) { return true; } else { return false; }; }))) { return false; } else { return true; }; })) + math.mean(trainYdataMean.map(function(value, index, self) { let newItem = ((trainYdataMean[index] % (math.mean(trainYdataMean.map(function(value, index, self) { let newItem = parseFloat(trainYdataMean[index] / trainXdataMean[index] ** 2); return newItem; })) * trainXdataMean[index] ** 2)) * (math.mean(trainYdataMean.map(function(value, index, self) { let newItem = parseFloat(trainYdataMean[index] / trainXdataMean[index] ** 2); return newItem; })) * trainXdataMean[index] ** 2)); return newItem; }).filter(function(value, index, self) { if ((index === self.findIndex((item, index, self) => { if (item === math.min(self)) { return true; } else { return false; }; })) || (index === self.findIndex((item, index, self) => { if (item === math.max(self)) { return true; } else { return false; }; }))) { return false; } else { return true; }; })) + math.mean(trainYdataMean.map(function(value, index, self) { let newItem = ((trainYdataMean[index] % (math.mean(trainYdataMean.map(function(value, index, self) { let newItem = parseFloat(trainYdataMean[index] / trainXdataMean[index] ** 3); return newItem; })) * trainXdataMean[index] ** 3)) * (math.mean(trainYdataMean.map(function(value, index, self) { let newItem = parseFloat(trainYdataMean[index] / trainXdataMean[index] ** 3); return newItem; })) * trainXdataMean[index] ** 3)); return newItem; }).filter(function(value, index, self) { if ((index === self.findIndex((item, index, self) => { if (item === math.min(self)) { return true; } else { return false; }; })) || (index === self.findIndex((item, index, self) => { if (item === math.max(self)) { return true; } else { return false; }; }))) { return false; } else { return true; }; }))),
//             // //     math.mean(Array.from({ length: parseInt(trainYdataMean.length) }, function(_, index) { return index; }).map(function(value, index, self) { let newItem = parseFloat(trainYdataMean[value] / trainXdataMean[value] ** 1); return newItem; }).filter(function(value, index, self) { if ((index === self.findIndex((item, index, self) => { if (item === math.min(self)) { return true; } else { return false; }; })) || (index === self.findIndex((item, index, self) => { if (item === math.max(self)) { return true; } else { return false; }; }))) { return false; } else { return true; }; })),
//             // //     math.mean(Array.from({ length: parseInt(trainYdataMean.length) }, function(_, index) { return index; }).map(function(value, index, self) { let newItem = parseFloat(trainYdataMean[value] / trainXdataMean[value] ** 2); return newItem; }).filter(function(value, index, self) { if ((index === self.findIndex((item, index, self) => { if (item === math.min(self)) { return true; } else { return false; }; })) || (index === self.findIndex((item, index, self) => { if (item === math.max(self)) { return true; } else { return false; }; }))) { return false; } else { return true; }; })),
//             // //     math.mean(Array.from({ length: parseInt(trainYdataMean.length) }, function(_, index) { return index; }).map(function(value, index, self) { let newItem = parseFloat(trainYdataMean[value] / trainXdataMean[value] ** 3); return newItem; }).filter(function(value, index, self) { if ((index === self.findIndex((item, index, self) => { if (item === math.min(self)) { return true; } else { return false; }; })) || (index === self.findIndex((item, index, self) => { if (item === math.max(self)) { return true; } else { return false; }; }))) { return false; } else { return true; }; }))
//             // // ];
//             // if (request_POST_JSON.hasOwnProperty("Pdata_0")) {
//             //     if (request_POST_JSON["Pdata_0"].length > 0) {
//             //         // Pdata_0 = request_POST_JSON["Pdata_0"];
//             //         Pdata_0 = new Array;
//             //         for (let i = 0; i < request_POST_JSON["Pdata_0"].length; i++) {
//             //             if (Object.prototype.toString.call(request_POST_JSON["Pdata_0"][i]).toLowerCase() === '[object number]') {
//             //                 Pdata_0.push(parseFloat(request_POST_JSON["Pdata_0"][i]));
//             //             } else if (Object.prototype.toString.call(request_POST_JSON["Pdata_0"][i]).toLowerCase() === '[object string]') {
//             //                 if (request_POST_JSON["Pdata_0"][i] === "+Base.Inf" || request_POST_JSON["Pdata_0"][i] === "+Inf" || request_POST_JSON["Pdata_0"][i] === "+inf" || request_POST_JSON["Pdata_0"][i] === "+Infinity" || request_POST_JSON["Pdata_0"][i] === "+infinity" || request_POST_JSON["Pdata_0"][i] === "Base.Inf" || request_POST_JSON["Pdata_0"][i] === "Inf" || request_POST_JSON["Pdata_0"][i] === "inf" || request_POST_JSON["Pdata_0"][i] === "Infinity" || request_POST_JSON["Pdata_0"][i] === "infinity") {
//             //                     Pdata_0.push(+Infinity);
//             //                 } else if (request_POST_JSON["Pdata_0"][i] === "-Base.Inf" || request_POST_JSON["Pdata_0"][i] === "-Inf" || request_POST_JSON["Pdata_0"][i] === "-inf" || request_POST_JSON["Pdata_0"][i] === "-Infinity" || request_POST_JSON["Pdata_0"][i] === "-infinity") {
//             //                     Pdata_0.push(-Infinity);
//             //                 } else {
//             //                     Pdata_0.push(parseFloat(request_POST_JSON["Pdata_0"][i]));
//             //                     // Pdata_0.push(request_POST_JSON["Pdata_0"][i]);
//             //                 };
//             //             } else {
//             //                 Pdata_0.push(request_POST_JSON["Pdata_0"][i]);
//             //             };
...
//             //             if (Object.prototype.toString.call(request_POST_JSON["Plower"][i]).toLowerCase() === '[object number]') {
//             //                 Plower.push(parseFloat(request_POST_JSON["Plower"][i]));
//             //             } else if (Object.prototype.toString.call(request_POST_JSON["Plower"][i]).toLowerCase() === '[object string]') {
//             //                 if (request_POST_JSON["Plower"][i] === "+Base.Inf" || request_POST_JSON["Plower"][i] === "+Inf" || request_POST_JSON["Plower"][i] === "+inf" || request_POST_JSON["Plower"][i] === "+Infinity" || request_POST_JSON["Plower"][i] === "+infinity" || request_POST_JSON["Plower"][i] === "Base.Inf" || request_POST_JSON["Plower"][i] === "Inf" || request_POST_JSON["Plower"][i] === "inf" || request_POST_JSON["Plower"][i] === "Infinity" || request_POST_JSON["Plower"][i] === "infinity") {
//             //                     Plower.push(+Infinity);
//             //                 } else if (request_POST_JSON["Plower"][i] === "-Base.Inf" || request_POST_JSON["Plower"][i] === "-Inf" || request_POST_JSON["Plower"][i] === "-inf" || request_POST_JSON["Plower"][i] === "-Infinity" || request_POST_JSON["Plower"][i] === "-infinity") {
//             //                     Plower.push(-Infinity);
//             //                 } else {
//             //                     Plower.push(parseFloat(request_POST_JSON["Plower"][i]));
//             //                     // Plower.push(request_POST_JSON["Plower"][i]);
//             //                 };
//             //             } else {
//             //                 Plower.push(request_POST_JSON["Plower"][i]);
//             //             };
...
//             //             if (Object.prototype.toString.call(request_POST_JSON["Pupper"][i]).toLowerCase() === '[object number]') {
//             //                 Pupper.push(parseFloat(request_POST_JSON["Pupper"][i]));
//             //             } else if (Object.prototype.toString.call(request_POST_JSON["Pupper"][i]).toLowerCase() === '[object string]') {
//             //                 if (request_POST_JSON["Pupper"][i] === "+Base.Inf" || request_POST_JSON["Pupper"][i] === "+Inf" || request_POST_JSON["Pupper"][i] === "+inf" || request_POST_JSON["Pupper"][i] === "+Infinity" || request_POST_JSON["Pupper"][i] === "+infinity" || request_POST_JSON["Pupper"][i] === "Base.Inf" || request_POST_JSON["Pupper"][i] === "Inf" || request_POST_JSON["Pupper"][i] === "inf" || request_POST_JSON["Pupper"][i] === "Infinity" || request_POST_JSON["Pupper"][i] === "infinity") {
//             //                     Pupper.push(+Infinity);
//             //                 } else if (request_POST_JSON["Pupper"][i] === "-Base.Inf" || request_POST_JSON["Pupper"][i] === "-Inf" || request_POST_JSON["Pupper"][i] === "-inf" || request_POST_JSON["Pupper"][i] === "-Infinity" || request_POST_JSON["Pupper"][i] === "-infinity") {
//             //                     Pupper.push(-Infinity);
//             //                 } else {
//             //                     Pupper.push(parseFloat(request_POST_JSON["Pupper"][i]));
//             //                     // Pupper.push(request_POST_JSON["Pupper"][i]);
//             //                 };
//             //             } else {
//             //                 Pupper.push(request_POST_JSON["Pupper"][i]);
//             //             };
...
//             // // let weight = new Array(parseInt(trainYdataMean.length)).fill(parseFloat(1.0));
//             // // // 使用高斯核賦權法;
...
//             // // if ((parseInt(target) >= parseInt(1)) && (parseInt(target) <= parseInt(trainYdataMean.length))) {
//             // //     for (let i = 0; i < trainYdataMean.length; i++) {
//             // //         let wei = parseFloat(1.0);
//             // //         if ((Object.prototype.toString.call(trainYdataMean[i]).toLowerCase() === '[object number]') && (Object.prototype.toString.call(trainYdataMean[parseInt(parseInt(target) - parseInt(1))]).toLowerCase() === '[object number]')) {
//             // //             if (parseFloat(trainYdataMean[parseInt(parseInt(target) - parseInt(1))]) === parseFloat(0.0)) {
//             // //                 wei = parseFloat(1.0);
//             // //             } else {
//             // //                 // math.exp((trainYdataMean[i] / trainYdataMean[parseInt(parseInt(target) - parseInt(1))] - 1)**2 / ((-2) * af**2));
//             // //                 // math.pow(math.e, ((trainYdataMean[i] / trainYdataMean[parseInt(parseInt(target) - parseInt(1))] - 1)**2 / ((-2) * af**2)));
//             // //                 wei = parseFloat(math.exp(parseFloat(parseInt(-1) * parseFloat(parseFloat(parseFloat(parseFloat(parseFloat(trainYdataMean[i]) / parseFloat(trainYdataMean[parseInt(parseInt(target) - parseInt(1))])) - parseInt(1)) ** parseInt(2)) / parseFloat(parseInt(2) * parseFloat(parseFloat(af) ** parseInt(2)))))));
//             // //             };
//             // //         };
//             // //         weight[i] = wei;
//             // //         // weight.push(wei);
...
//             // //     };
//             // // };
//             // if (request_POST_JSON.hasOwnProperty("weight")) {
//             //     if (request_POST_JSON["weight"].length > 0) {
//             //         // weight = request_POST_JSON["weight"];
//             //         weight = new Array;
//             //         for (let i = 0; i < request_POST_JSON["weight"].length; i++) {
//             //             if (Object.prototype.toString.call(request_POST_JSON["weight"][i]).toLowerCase() === '[object number]') {
//             //                 weight.push(parseFloat(request_POST_JSON["weight"][i]));
//             //             } else if (Object.prototype.toString.call(request_POST_JSON["weight"][i]).toLowerCase() === '[object string]') {
//             //                 if (request_POST_JSON["weight"][i] === "+Base.Inf" || request_POST_JSON["weight"][i] === "+Inf" || request_POST_JSON["weight"][i] === "+inf" || request_POST_JSON["weight"][i] === "+Infinity" || request_POST_JSON["weight"][i] === "+infinity" || request_POST_JSON["weight"][i] === "Base.Inf" || request_POST_JSON["weight"][i] === "Inf" || request_POST_JSON["weight"][i] === "inf" || request_POST_JSON["weight"][i] === "Infinity" || request_POST_JSON["weight"][i] === "infinity") {
//             //                     weight.push(+Infinity);
//             //                 } else if (request_POST_JSON["weight"][i] === "-Base.Inf" || request_POST_JSON["weight"][i] === "-Inf" || request_POST_JSON["weight"][i] === "-inf" || request_POST_JSON["weight"][i] === "-Infinity" || request_POST_JSON["weight"][i] === "-infinity") {
//             //                     weight.push(-Infinity);
//             //                 } else {
//             //                     weight.push(parseFloat(request_POST_JSON["weight"][i]));
//             //                     // weight.push(request_POST_JSON["weight"][i]);
//             //                 };
//             //             } else {
//             //                 weight.push(request_POST_JSON["weight"][i]);
//             //             };
...
//             if (typeof (request_POST_JSON) === 'object' && Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object object]' && !(request_POST_JSON.length) && JSON.stringify(request_POST_JSON) !== '{}') {

//                 // let fitResult = Polynomial3_fit(
//                 //     {"Xdata": trainXdata, "Ydata": trainYdata},  // training_data;
//                 //     {"Xdata": testXdata, "Ydata": testYdata},  // testing_data;
//                 //     {
//                 //         "Pdata_0": Pdata_0,
//                 //         "Plower": Plower,
//                 //         "Pupper": Pupper,
...
//                 //     }
//                 // );
//                 // // console.log(fitResult["Coefficient"]);
//                 // // console.log(fitResult);
//                 // // console.log(fitResult["testData"]);

//                 // for (let keyFitResult in fitResult) {
//                 //     if (fitResult.hasOwnProperty(keyFitResult)) {
//                 //         // console.log(`${keyFitResult}: ${fitResult[keyFitResult]}`);
//                 //         response_data_JSON[keyFitResult] = fitResult[keyFitResult];
//                 //     };
//                 // };
//                 // // Object.keys(fitResult).forEach(function (key) {
//                 // //     console.log(`${key}: ${fitResult[key]}`);
//                 // //     response_data_JSON[key] = fitResult[key];
//                 // // });
...
//         case "/LC4PFit": {
//             // 讀取用戶端（前端 Client）發送的請求數據（Request），並進行四參數邏輯回歸擬合（5 parameter Logistic fitting）運算，並向用戶端（前端 Client）返回運算結果數據;
//             // 客戶端或瀏覽器請求 url = http://127.0.0.1:10001/LC4PFit?Key=username:password&algorithmUser=username&algorithmPass=password&algorithmName=LC4PFit
...
let interface_Function_name_str = "http_Server";  // "file_Monitor" or "http_Server" or "http_Client" or "Interface_file_Monitor" or "Interface_http_Server" or "Interface_http_Client";
...
// 將從配置文檔（config.txt）内讀取到的參數值，傳遞給使用 require() 方法導入的自定義路由（Router）模組「Router.js」内的變量，刷新賦值;
Router.setValue({"webPath": webPath, "Key": Key, "Session": Session});
...
// const do_Response_Router = Router.do_Response_Router;  // 使用「Router.js」模組中的成員「do_Response_Router(response_status, response_headers, response_POST_String, callback)」函數, 用於處理從服務端（Server）返回的響應值數據（Response）的執行函數，即路由（Router）功能;
// if (do_Function === null) {
//     do_Function = do_data;  // function (argument) { return argument; };  // 函數對象字符串，用於接收執行數據處理功能的函數 "do_data";
// };
// if (do_Request === null) {
//     do_Request = do_Request_Router;  // function (argument) { return argument; };  // 用於接收執行對根目錄(/)的 GET 或 POST 請求處理功能的函數 "do_Request_Router";
// };
// if (do_Response === null) {
//     do_Response = do_Response_Router;  // function (argument) { return argument; };  // 用於接收執行處理響應值（response）處理功能的函數 "do_Response_Router";
// };
...
if (interface_Function_name_str === "file_Monitor" || interface_Function_name_str === "Interface_file_Monitor") {
...
} else if (interface_Function_name_str === "http_Server" || interface_Function_name_str === "Interface_http_Server") {
    result_Array = interface_Function({
        "host": host,
        "port": port,
        "number_cluster_Workers": number_cluster_Workers,
    ...
    //     "number_cluster_Workers": number_cluster_Workers,
    // });
} else if (interface_Function_name_str === "http_Client" || interface_Function_name_str === "Interface_http_Client") {
    interface_Function({
        "Host": Host,
        "Port": Port,
        "URL": URL,
