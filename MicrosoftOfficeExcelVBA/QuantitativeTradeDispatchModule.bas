Attribute VB_Name = "QuantitativeTradeDispatchModule"
...
Sub MenuSetup()
    ...
    '插入數量化交易計算 menuQuantitativeTrading 操作菜單
    '判斷是否自定義的菜單已存在，菜單不允許重名，如果已存在，則先刪除
    'Application.CommandBars("Worksheet Menu Bar").Controls("數量化交易 Quantitative Trading").Delete
    Dim ctl As CommandBarControl
    For Each ctl In Application.CommandBars("Worksheet Menu Bar").Controls
        If ctl.Caption = "數量化交易 Quantitative Trading" Then ctl.Delete
    Next ctl

    Dim menuQuantitativeTrading As CommandBarPopup  '頂層菜單，CommandBarPopup 表示下拉菜單類型
    Set menuQuantitativeTrading = menuBar.Controls.Add(Type:=msoControlPopup, Temporary:=True)  '參數 Type:=msoControlPopup 表示下拉菜單類型；參數 Temporary:=True 表示臨時新增菜單，Excel 關閉後會自動刪除。
    menuQuantitativeTrading.Caption = "數量化交易 Quantitative Trading"  '自定義的頂層菜單名
    menuQuantitativeTrading.TooltipText = "股票低頻數量化交易操作用戶端"  '菜單提示文字

    Dim menuQuantitativeTradingServer As CommandBarButton  '一級子菜單，CommandBarButton 表示按鈕類型。 'CommandBarPopup  '一級子菜單，CommandBarPopup 表示下拉菜單類型
    Set menuQuantitativeTradingServer = menuQuantitativeTrading.Controls.Add(Type:=msoControlButton)  '參數 Type:=msoControlButton 表示按鈕類型。
    menuQuantitativeTradingServer.Caption = "數量化交易運算伺服器 Quantitative Trading server"  '自定義的一級子菜單名
    'menuQuantitativeTradingServer.Caption = "數量化交易運算 Quantitative Trading server(&S)"  '自定義的一級子菜單名，參數 (&S) 表示配置的啓動快捷鍵，括號中的 &S 表示快捷鍵：Ctrl + Shift + S
    menuQuantitativeTradingServer.TooltipText = "對應不同的數量化交易策略模型，提供相應的計算方法的伺服器"  '菜單提示文字
    menuQuantitativeTradingServer.Style = msoButtonIconAndCaption  '菜單樣式（圖標加文字）
    menuQuantitativeTradingServer.FaceId = 263  '圖標代號
    'menuQuantitativeTradingServer.ShortcutText = "Ctrl + Shift + S"  '提示快捷鍵的顯示字符串：Ctrl + Shift + S
    menuQuantitativeTradingServer.OnAction = "runTradingAlgorithmServer"  '左鍵單擊後執行的宏名稱字符串
    menuQuantitativeTradingServer.BeginGroup = True  '添加分割綫



    Dim menuQuantitativeTradingPanel As CommandBarButton  '一級子菜單，CommandBarButton 表示按鈕類型
    Set menuQuantitativeTradingPanel = menuQuantitativeTrading.Controls.Add(Type:=msoControlButton)  '參數 Type:=msoControlButton 表示按鈕類型。
    menuQuantitativeTradingPanel.Caption = "人機交互介面 operation panel" '自定義的一級子菜單名
    'menuQuantitativeTradingPanel.Caption = "人機交互介面 operation panel(&S)" '自定義的一級子菜單名，參數 (&S) 表示配置的啓動快捷鍵，括號中的 &S 表示快捷鍵：Ctrl + Shift + S
    menuQuantitativeTradingPanel.TooltipText = "操控股票低頻數量化交易的人機交互介面操作面板"  '菜单提示文字
    menuQuantitativeTradingPanel.Style = msoButtonIconAndCaption  '菜單樣式（圖標加文字）
    menuQuantitativeTradingPanel.FaceId = 263  '圖標代號
    'menuQuantitativeTradingPanel.ShortcutText = "Ctrl + Shift + S"  '提示快捷鍵的顯示字符串：Ctrl + Shift + S
    menuQuantitativeTradingPanel.OnAction = "QuantitativeTradeDispatchModule.TradingAlgorithm"  '左鍵單擊後執行的宏名稱字符串
    menuQuantitativeTradingPanel.BeginGroup = True  '添加分割綫


    'Dim menuQuantitativeTradingPanel As CommandBarPopup  '一級子菜單，CommandBarPopup 表示下拉菜單類型
    'Set menuQuantitativeTradingPanel = menuQuantitativeTrading.Controls.Add(Type:=msoControlPopup)  '參數 Type:=msoControlPopup 表示下拉菜單類型。
    'menuQuantitativeTradingPanel.Caption = "人機交互介面 operation panel"  '自定義的一級子菜單名
    'menuQuantitativeTradingPanel.TooltipText = "操控股票低頻數量化交易的人機交互介面操作面板"  '菜單提示文字

    'Dim menuQuantitativeTradingPanelTest As CommandBarButton  '二級子菜單，CommandBarButton 表示按鈕類型
    'Set menuQuantitativeTradingPanelTest = menuQuantitativeTradingPanel.Controls.Add(Type:=msoControlButton)  '參數 Type:=msoControlButton 表示按鈕類型。
    'menuQuantitativeTradingPanelTest.Caption = "示例工程 test" '自定義的二級子菜單名
    ''menuQuantitativeTradingPanelTest.Caption = "示例工程 test(&T)" '自定義的二級子菜單名，參數 (&T) 表示配置的啓動快捷鍵，括號中的 &T 表示快捷鍵：Ctrl + Shift + T
    'menuQuantitativeTradingPanelTest.TooltipText = "操控股票低頻數量化交易策略 test 的人機交互介面操作面板"  '菜单提示文字
    'menuQuantitativeTradingPanelTest.Style = msoButtonIconAndCaption  '菜單樣式（圖標加文字）
    'menuQuantitativeTradingPanelTest.FaceId = 263  '圖標代號
    ''menuQuantitativeTradingPanelTest.ShortcutText = "Ctrl + Shift + T"  '提示快捷鍵的顯示字符串：Ctrl + Shift + T
    'menuQuantitativeTradingPanelTest.OnAction = "testTradingAlgorithm"  '左鍵單擊後執行的宏名稱字符串
    'menuQuantitativeTradingPanelTest.BeginGroup = True  '添加分割綫

End Sub


Sub runTradingAlgorithmServer()

    '聲明變量
    Dim codeScript As String, shellPIDvalue As Integer

    '檢查是否已經開啓過服務器，如果已經開啓則退出當前過程，如果未開啓過則執行開啓服務器
    If shellPIDdict.Exists("QuantitativeTradingServer") Then

        Dim oWMT As Object, oProcess As Object
        Set oWMT = GetObject("winmgmts:")
        For Each oProcess In oWMT.InstancesOf("Win32_Process")
            If oProcess.ProcessID = shellPIDdict.Item("QuantitativeTradingServer") Then
                Debug.Print "running Quantitative Trading server PID: " & oProcess.ProcessID
                'Application.CommandBars("Worksheet Menu Bar").Controls("數量化交易 Quantitative Trading").Controls("數量化交易運算服務器 Quantitative Trading server").Caption = "數量化交易運算服務器 Quantitative Trading server(running)"   '變更顯示文字
                ''Application.CommandBars("Worksheet Menu Bar").Controls("數量化交易 Quantitative Trading").Controls("數量化交易運算服務器 Quantitative Trading server").Font.ColorIndex = 15   '變更顯示字體
                ''Application.CommandBars("Worksheet Menu Bar").Controls("數量化交易 Quantitative Trading").Controls("數量化交易運算服務器 Quantitative Trading server").Enabled = False   '禁用
                Exit Sub
            End If
        Next
        Set oWMT = Nothing
        ...
        'codeScript = CStr(CStr(ThisWorkbook.Path) & "\QuantitativeTradingPython\Scripts\python.exe" & " " & CStr(ThisWorkbook.Path) & "\QuantitativeTradingPython\src\QuantitativeTradingServer.py")  '開啓服務器的命令行控制臺 cmd 命令全名，函數：CStr(ThisWorkbook.Path) 表示 Excel 文檔（.xlsm）所在的路徑;
        'Debug.Print codeScript
        'codeScript = "C:\QuantitativeTrading\Julia\Julia-1.10.10\bin\julia.exe -p 4 --project=C:/QuantitativeTrading/QuantitativeTradingJulia/ C:/QuantitativeTrading/QuantitativeTradingJulia/src/QuantitativeTradingServer.jl"  '開啓服務器的命令行控制臺 cmd 命令全名
        codeScript = "C:\QuantitativeTrading\Julia\Julia-1.10.10\bin\julia.exe --project=C:/QuantitativeTrading/QuantitativeTradingJulia/ C:/QuantitativeTrading/QuantitativeTradingJulia/src/QuantitativeTradingServer.jl"  '開啓服務器的命令行控制臺 cmd 命令全名
        'codeScript = "C:\QuantitativeTrading\QuantitativeTradingPython\Scripts\python.exe C:/QuantitativeTrading/QuantitativeTradingPython/src/QuantitativeTradingServer.py"  '開啓服務器的命令行控制臺 cmd 命令全名
        'codeScript = "C:\QuantitativeTrading\Python\Python311\python.exe C:/QuantitativeTrading/QuantitativeTradingPython/src/QuantitativeTradingServer.py"  '開啓服務器的命令行控制臺 cmd 命令全名
        ...
        Debug.Print "running Quantitative Trading server PID: " & shellPIDvalue
        'Application.CommandBars("Worksheet Menu Bar").Controls("數量化交易 Quantitative Trading").Controls("數量化交易運算服務器 Quantitative Trading server").Caption = "數量化交易運算服務器 Quantitative Trading server(running)"   '變更顯示文字
        ''Application.CommandBars("Worksheet Menu Bar").Controls("數量化交易 Quantitative Trading").Controls("數量化交易運算服務器 Quantitative Trading server").Font.ColorIndex = 15   '變更顯示字體
        ''Application.CommandBars("Worksheet Menu Bar").Controls("數量化交易 Quantitative Trading").Controls("數量化交易運算服務器 Quantitative Trading server").Enabled = False   '禁用
        ...
        If shellPIDdict.Exists("QuantitativeTradingServer") Then
            shellPIDdict.Item("QuantitativeTradingServer") = shellPIDvalue
        Else
            shellPIDdict.Add "QuantitativeTradingServer", shellPIDvalue
        End If
    ...
    End If
    'Debug.Print shellPIDdict.Item("QuantitativeTradingServer")

End Sub

'Sub testTradingAlgorithm()
'
'    ' MsgBox "統計運算示例工程 test"
'
'
'    On Error Resume Next
'
...
'    moduleName = "testTradingAlgorithmModule"  '統計算法模塊的自定義命名
'    modulePath = "C:\Criss\vba\Statistics\TradingAlgorithmServer\test"  '統計算法模塊的保存路徑
'    moduleFileName = "testTradingAlgorithmModule.bas"  '統計算法模塊的文檔名
'    moduleFile = modulePath & "\" & moduleFileName  '統計算法模塊文檔的路徑全名
'
...
'
'            'Debug.Print "Statistics Algorithm ( " & moduleName & " ) source file: " & moduleFile
'
'            '調用自定義子過程導入模塊
...
'        Else
'
'            Debug.Print "Statistics Algorithm ( " & moduleName & " ) error, Source file is Nothing."
'
'            'If ThisWorkbook.VBProject.VBComponents(moduleName) Is Nothing Then
'            '    MsgBox "Statistics Algorithm ( " & moduleName & " ) error, Source file ( " & moduleFile & " ) is Nothing."
'            'End If
'
'            Exit Sub
'
...
'        End If
'
'        '執行導入模塊 testTradingAlgorithmModule 中的自定義子過程 Sub testTradingAlgorithm()，函數 .Evaluate () 表示將字符串解析為代碼並執行
'        'Application.Run "testTradingAlgorithmModule.testTradingAlgorithm"
'        Application.Run moduleName & "." & Split(moduleName, "Module")(0)
'        'Call testTradingAlgorithmModule.testTradingAlgorithm
'        'Application.Evaluate ("Call testTradingAlgorithmModule.testTradingAlgorithm()")  '執行導入模塊中的自定義子過程 testTradingAlgorithm，函數 .Evaluate () 表示將字符串解析為代碼並執行
'        'Application.Evaluate ("Call " & moduleName & "." & Split(moduleName, "Module")(0) & "()")  '執行導入模塊中的自定義子過程 testTradingAlgorithm，函數 .Evaluate () 表示將字符串解析為代碼並執行
'
'        ''判斷使用的爬蟲模塊名稱，可以取值：("testTradingAlgorithmModule", "LC5PTradingAlgorithmModule")
'        'Select Case moduleName
'
'        '    Case "testTradingAlgorithmModule"
'
'        '        Call testTradingAlgorithmModule.testTradingAlgorithm  '執行導入模塊中的自定義子過程 testTradingAlgorithm，函數
'        '        'ThisWorkbook.VBProject.VBComponents("testTradingAlgorithmModule").Controls("testTradingAlgorithm")
'
'        '        'Exit Sub
'
'        '    Case "InterpolationTradingAlgorithmModule"
'
'        '    Case "Polynomial3TradingAlgorithmModule"
'
'        '    Case "LC5PTradingAlgorithmModule"
'
'        '        'Call LC5PTradingAlgorithmModule.LC5PTradingAlgorithm  '執行導入模塊中的自定義子過程 LC5PTradingAlgorithm，函數
'        '        ''ThisWorkbook.VBProject.VBComponents("LC5PTradingAlgorithmModule").Controls("LC5PTradingAlgorithm")
'
'        '        'Exit Sub
'
'        '    Case Else
...
'        'Application.Evaluate ("Call " & moduleName & "." & Split(moduleName, "Module")(0) & "()")  '執行導入模塊中的自定義子過程 testTradingAlgorithm，函數 .Evaluate () 表示將字符串解析為代碼並執行
'
'        ''判斷使用的統計算法模塊名稱，可以取值：("testTradingAlgorithmModule", "LC5PTradingAlgorithmModule")
'        'Select Case moduleName
'
'        '    Case "testTradingAlgorithmModule"
'
...
Sub TradingAlgorithm()
    ...
    '自定義的待導入的統計算法模塊
    Dim moduleName As String, modulePath As String, moduleFileName As String, moduleFile As String
    moduleName = "TradingAlgorithmModule"  '統計算法模塊的自定義命名
    modulePath = CStr(ThisWorkbook.Path)  'Excel 文檔（.xlsm）所在的路徑，統計算法模塊的保存路徑
    'modulePath = "C:\Criss\vba\QuantitativeTrading\TradingAlgorithmServer"  '統計算法模塊的保存路徑
    moduleFileName = "TradingAlgorithmModule.bas"  '統計算法模塊的文檔名
    moduleFile = modulePath & "\" & moduleFileName  '統計算法模塊文檔的路徑全名
    'codeScript = CStr(CStr(ThisWorkbook.Path) & "\" & CStr(moduleFileName)) '統計算法模塊文檔的路徑全名，函數：CStr(ThisWorkbook.Path) 表示 Excel 文檔（.xlsm）所在的路徑;
    'Debug.Print codeScript
    ...
    If ThisWorkbook.VBProject.VBComponents(moduleName) Is Nothing Then
        ...
        If FSO.FolderExists(modulePath) And FSO.FileExists(moduleFile) Then
            ...
            'Debug.Print "QuantitativeTrading Algorithm ( " & moduleName & " ) source file: " & moduleFile
        ...
        Else
            ...
            Debug.Print "QuantitativeTrading Algorithm ( " & moduleName & " ) error, Source file is Nothing."

            'If ThisWorkbook.VBProject.VBComponents(moduleName) Is Nothing Then
            '    MsgBox "QuantitativeTrading Algorithm ( " & moduleName & " ) error, Source file ( " & moduleFile & " ) is Nothing."
            'End If
        ...
        '執行導入模塊 TradingAlgorithmModule 中的自定義子過程 Sub TradingAlgorithm()，函數 .Evaluate () 表示將字符串解析為代碼並執行
        'Application.Run "TradingAlgorithmModule.TradingAlgorithm"
        Application.Run moduleName & "." & Split(moduleName, "Module")(0)
        'Call TradingAlgorithmModule.TradingAlgorithm
        'Application.Evaluate ("Call TradingAlgorithmModule.TradingAlgorithm()")  '執行導入模塊中的自定義子過程 TradingAlgorithm，函數 .Evaluate () 表示將字符串解析為代碼並執行
        'Application.Evaluate ("Call " & moduleName & "." & Split(moduleName, "Module")(0) & "()")  '執行導入模塊中的自定義子過程 TradingAlgorithm，函數 .Evaluate () 表示將字符串解析為代碼並執行

        ''判斷使用的統計算法模塊名稱，可以取值：("TradingAlgorithmModule")
        'Call TradingAlgorithmModule.TradingAlgorithm  '執行導入模塊中的自定義子過程 TradingAlgorithm，函數
        ''ThisWorkbook.VBProject.VBComponents("TradingAlgorithmModule").Controls("TradingAlgorithm")
        ''Exit Sub

        'QuantitativeTradeControlPanel.show  '顯示自定義的操作面板窗體控件
        'QuantitativeTradeControlPanel.Hide  '隱藏自定義的操作面板窗體控件
        'Unload QuantitativeTradeControlPanel
        'Load UserForm: QuantitativeTradeControlPanel
        'For i = 0 To QuantitativeTradeControlPanel.Controls.Count - 1
        '    DoEvents: Rem 語句 DoEvents 表示交出系統 CPU 控制權還給操作系統，也就是在此循環階段，用戶可以同時操作電腦的其它應用，而不是將程序挂起直到循環結束。
        'Next i

        'Exit Sub

        'Call TradingAlgorithm
...
Public Sub callRemoveModule()
    ...
    Else
        ...
        Debug.Print "No imported QuantitativeTrading algorithm module needs to be removed."
